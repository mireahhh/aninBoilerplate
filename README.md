...
# Выложить на гитхаб пейджс:

```
Дальше сначала установить зависимости ярна и созбрать сайт (первый билд долгий)
    yarn
    yarn build
...
И выставить:
    ...
    Branch: main (ВЕТКА_КОТОРУЮ_ПОКАЗЫВАТЬ)
    Folder: /docs (ПАПКА_СБОРКИ ; этот бойлер собирает в докс)
...
Переключиться на ветку
    git checkout Название
Пропушить в отпределённую ветку (не забыть гит адд)
    git push -u origin gh-pages
```
